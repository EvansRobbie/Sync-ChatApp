...
const SearchBar = () => {
  ...
  const handleClick = async () =>{
    ...
    const combinedId = currentUser && currentUser?.uid > user?.uid ? currentUser?.uid + user?.uid : user?.uid + currentUser?.uid
    try{
      ...
      if(!response.exists()){
        ...
        // create userChat
        const chatRef = doc(collection(db, 'userCharts'), currentUser?.uid);
